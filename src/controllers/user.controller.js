...
try {
  ...
  const updateAccountData = asyncHandler(async(req,res)=>{
    ...
    if(!fullName || !email){
      throw new ApiError(400,"get me the user and email")
    }
...
$set:{
  fullName,email
     ...
     return res
     .status(200)
     .json(new ApiResponse(200,user,"account details updata succesffully"))
...
$set:{
  ...
  const updateCovermage = asyncHandler(async(req,res)=>{
    const coverImageLocalPath = req.file?.path
    if(!coverImageLocalPath){
      throw new ApiError(400,"cover file is missing")
    }
